...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (editingId) {
    const record = Records.getById(editingId);
  ...
  document.getElementById('fDate').addEventListener('change', function() {
    ...
    if (!dateVal) return;
    const existing = Records.getByDate(currentUser.id, dateVal);
    if (existing) {
...
function populateForm(record) {
  ...
  ['fWalking','fRunning','fSquats','fPushups','fSitups','fWater','fFasting'].forEach(id => {
    const progressId = 'progress' + id.replace('f','').charAt(0).toUpperCase() + id.replace('f','').slice(1);
    const pctId = 'pct' + id.replace('f','').charAt(0).toUpperCase() + id.replace('f','').slice(1);
    updateProgress(id, progressId, pctId, id.slice(1).toLowerCase());
  });
...
function handleSave(e) {
  ...
  if (!editingId) {
    const dup = Records.getByDate(currentUser.id, dateVal);
    if (dup) {
