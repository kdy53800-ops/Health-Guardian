...
async function confirmDelete() {
  ...
  try {
    await Records.deleteAsync(pendingDeleteId, currentUser.id);
    userRecords = userRecords.filter(record => record.id !== pendingDeleteId);
  } catch (error) {
    console.error('[HistoryDelete]', error);
    showToast(error.message || '기록 삭제 중 오류가 발생했습니다.', 'error');
    closeConfirm();
    return;
  }
  closeConfirm();
